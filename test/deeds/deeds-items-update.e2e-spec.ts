...
describe('DeedsController (e2e) - PATCH /deeds/:category/items/:deed_item_id', () => {
  ...
  it('-> 401, not 500, when Authorization header missing', async () => {
    const response = await request(app.getHttpServer())
      .patch(`/deeds/hasanaat/items/${deedItemId}`)
      .send(validPayload)
      .expect(401);
  ...
  it('-> 400, not 500, when deed_item_id is not numeric', async () => {
    ...
    const response = await request(app.getHttpServer())
      .patch('/deeds/hasanaat/items/not-a-number')
  ...
  it('-> 400, not 500, when category is invalid', async () => {
    jwtVerifyAsyncMock.mockResolvedValueOnce(accessTokenPayload);

    const response = await request(app.getHttpServer())
      .patch(`/deeds/invalid/items/${deedItemId}`)
      .set('Authorization', 'Bearer access-token')
      .send(validPayload)
      .expect(400);
  ...
  it('-> 400, not 500, when hide_type is invalid', async () => {
    jwtVerifyAsyncMock.mockResolvedValueOnce(accessTokenPayload);

    const response = await request(app.getHttpServer())
      .patch(`/deeds/hasanaat/items/${deedItemId}`)
      .set('Authorization', 'Bearer access-token')
      .send({ hide_type: 'invalid_type' })
      .expect(400);
  ...
  it('-> 400, not 500, when payload contains unknown fields', async () => {
    ...
    const response = await request(app.getHttpServer())
      .patch(`/deeds/hasanaat/items/${deedItemId}`)
      .set('Authorization', 'Bearer access-token')
      .send({ ...validPayload, extra_prop: true })
      .expect(400);
  ...
  it('-> 404, not 500, when deed item not found', async () => {
    jwtVerifyAsyncMock.mockResolvedValueOnce(accessTokenPayload);
    postgresQueryMock
  ...
  it('-> 204 updates deed item on happy path with all fields', async () => {
    jwtVerifyAsyncMock.mockResolvedValueOnce(accessTokenPayload);
    postgresQueryMock
      .mockResolvedValueOnce([{ deed_id: 5 }])
    ...
    const [, categoryParams] = postgresQueryMock.mock.calls[0];
    expect(categoryParams).toEqual([1, 'hasanaat']);
    ...
    expect(updateSql).toContain('hide_type = $3');
    expect(updateParams).toEqual([validPayload.name, validPayload.description, validPayload.hide_type, deedItemId, 5]);
  });

  it('-> 204 updates deed item with partial fields (name only)', async () => {
    jwtVerifyAsyncMock.mockResolvedValueOnce(accessTokenPayload);
    postgresQueryMock
      .mockResolvedValueOnce([{ deed_id: 5 }])
      .mockResolvedValueOnce([{ deed_item_id: deedItemId }]);
    ...
    expect(updateSql).not.toContain('hide_type =');
    expect(updateParams).toEqual(['Only Name', deedItemId, 5]);
  });
});
