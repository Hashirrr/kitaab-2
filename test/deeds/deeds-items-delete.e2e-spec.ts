...
describe('DeedsController (e2e) - DELETE /deeds/:category/items/:deed_item_id', () => {
  ...
  it('-> 401, not 500, when Authorization header missing', async () => {
    const response = await request(app.getHttpServer())
      .delete(`/deeds/hasanaat/items/${deedItemId}`)
      .expect(401);
  ...
  it('-> 400, not 500, when deed_item_id is not numeric', async () => {
    ...
    const response = await request(app.getHttpServer())
      .delete('/deeds/hasanaat/items/not-a-number')
  ...
  it('-> 400, not 500, when category is invalid', async () => {
    ...
    const response = await request(app.getHttpServer())
      .delete(`/deeds/invalid/items/${deedItemId}`)
      .set('Authorization', 'Bearer access-token')
      .expect(400);
  ...
  it('-> 204 deletes deed item on happy path', async () => {
    jwtVerifyAsyncMock.mockResolvedValueOnce(accessTokenPayload);
    postgresQueryMock
      .mockResolvedValueOnce([{ deed_id: 5 }])
    ...
    const [, deedCategoryParams] = postgresQueryMock.mock.calls[0];
    expect(deedCategoryParams).toEqual([1, 'hasanaat']);

    const [, deleteParams] = postgresQueryMock.mock.calls[1];
    expect(deleteParams).toEqual([deedItemId, 5]);
  });
});
