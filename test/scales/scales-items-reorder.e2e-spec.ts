...
describe('ScalesController (e2e) - PATCH /scales/:deed_item_id/items/display-order', () => {
  ...
  it('-> 401, not 500, when Authorization header missing', async () => {
    const response = await request(app.getHttpServer())
      .patch(`/scales/${deedItemId}/items/display-order`)
      .send({ display_order: [1, 2] })
      .expect(401);
  ...
  it('-> 400, not 500, when deed_item_id is not numeric', async () => {
    ...
    const response = await request(app.getHttpServer())
      .patch('/scales/not-a-number/items/display-order')
  ...
  it('-> 400, not 500, when display_order is empty', async () => {
    jwtVerifyAsyncMock.mockResolvedValueOnce(accessTokenPayload);
  ...
  it('-> 404, not 500, when one or more scale items not found', async () => {
    ...
    postgresQueryMock
      .mockResolvedValueOnce([{ deed_item_id: deedItemId }])
      .mockResolvedValueOnce([{ scale_items_id: 1 }]);
  ...
  it('-> 204 reorders scale items on happy path', async () => {
    jwtVerifyAsyncMock.mockResolvedValueOnce(accessTokenPayload);
    postgresQueryMock
      .mockResolvedValueOnce([{ deed_item_id: deedItemId }])
      .mockResolvedValueOnce([{ scale_items_id: 2 }, { scale_items_id: 1 }])
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([]);
    ...
    await request(app.getHttpServer())
      .patch(`/scales/${deedItemId}/items/display-order`)
      .set('Authorization', 'Bearer access-token')
      .send({ display_order: [2, 1] })
      .expect(204);

    expect(postgresTransactionMock).toHaveBeenCalledTimes(1);
    expect(postgresQueryMock).toHaveBeenCalledTimes(4);

    const [, checkParams] = postgresQueryMock.mock.calls[1];
    expect(checkParams).toEqual([deedItemId, [2, 1]]);

    const [firstUpdateSql, firstUpdateParams] = postgresQueryMock.mock.calls[2];
    expect(firstUpdateSql).toContain('+ 10000');
    expect(firstUpdateParams).toEqual([deedItemId, [2, 1]]);

    const [secondUpdateSql, secondUpdateParams] = postgresQueryMock.mock.calls[3];
    expect(secondUpdateSql).not.toContain('+ 10000');
    expect(secondUpdateParams).toEqual([deedItemId, [2, 1]]);
  });
});
