...
describe('ScalesController (e2e) - DELETE /scales/:deed_item_id/items/:scale_item_id', () => {
  ...
  it('-> 401, not 500, when Authorization header missing', async () => {
    const response = await request(app.getHttpServer())
      .delete(`/scales/${deedItemId}/items/${scaleItemId}`)
      .expect(401);
  ...
  it('-> 400, not 500, when deed_item_id is not numeric', async () => {
    ...
    const response = await request(app.getHttpServer())
      .delete(`/scales/not-a-number/items/${scaleItemId}`)
  ...
  it('-> 400, not 500, when scale_item_id is not numeric', async () => {
    ...
    const response = await request(app.getHttpServer())
      .delete(`/scales/${deedItemId}/items/not-a-number`)
      .set('Authorization', 'Bearer access-token')
      .expect(400);
  ...
  it('-> 204 deletes scale item on happy path', async () => {
    jwtVerifyAsyncMock.mockResolvedValueOnce(accessTokenPayload);
    postgresQueryMock
      .mockResolvedValueOnce([{ deed_item_id: deedItemId }])
    ...
    expect(deleteParams).toEqual([scaleItemId, deedItemId]);
  });
});
